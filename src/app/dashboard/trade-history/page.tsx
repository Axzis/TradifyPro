  ...
  const filteredTrades = useMemo(() => {
    ...
    return rawTrades
      .filter((trade) => {
        ...
        const tradeDate = safeToDate(trade.openDate);
        if (!tradeDate) return false;

        if (filterDateRange?.from && tradeDate < filterDateRange.from)
          return false;
      ...
      .sort(
        (a, b) =>
          (safeToDate(b.openDate)?.getTime() || 0) - (safeToDate(a.openDate)?.getTime() || 0)
      );
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                router.push(`/dashboard/edit-trade/${trade.id}`)
                              }
                            >
                              Edit Trade
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              className="text-red-500 focus:bg-red-50 focus:text-red-600 dark:focus:bg-red-900/50"
                              onSelect={(e) => {
                                e.preventDefault();
                                setSelectedTrade(trade);
                                openDeleteAlert();
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Aksi ini tidak dapat dibatalkan. Ini akan menghapus data trade
      ...
      <Dialog open={isDetailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedTrade?.position} trade pada{' '}
              {selectedTrade && safeToDate(selectedTrade.openDate)
                ? format(safeToDate(selectedTrade.openDate)!, 'dd MMMM yyyy')
                : '-'}
            </DialogDescription>
          </DialogHeader>
          {selectedTrade && (
            <div className="grid gap-6 py-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold">{selectedTrade.position}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Aset</p>
                  <p className="font-semibold">{selectedTrade.assetType}</p>
                </div>
              </div>
              <Separator />
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {selectedTrade.imageUrlBefore ? (
                      <a
                        href={selectedTrade.imageUrlBefore}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <Image
                          src={selectedTrade.imageUrlBefore}
                          alt="Screenshot sebelum masuk"
                          width={300}
                          height={200}
                  ...
                  <div>
                    ...
                    {selectedTrade.imageUrlAfter ? (
                      <a
                        href={selectedTrade.imageUrlAfter}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <Image
                          src={selectedTrade.imageUrlAfter}
                          alt="Screenshot sesudah keluar"
                          width={300}
                          height={200}
                          className="rounded-md border object-cover aspect-video"
                        />
